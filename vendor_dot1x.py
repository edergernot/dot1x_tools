'''Used for enabling dot1x on every Interface where a Mac - Address from a special Vendor is found.
It uses the seeddevice for initial Connection, Does a how CDP-Neighbor and stores the MGMT-IPs of all Cat 92000 in a list which will  then execute Check.
Then it does a show mac address and checks the vendor if the mac is not learned on a Ten Gig Port.
if the vendor is what we want to change it checks if Port is allready dot1x configured. If not it will configure the settings for dot1x and shut/no shut the interface and checks the authentication for that port.
It the Port is in Guest VLAN it gives Warning '''
...
def dot1x_work(IP):
    ...
    macs=ssh.send_command("show mac address-table", use_textfsm=True)
    for mac in macs:
        #print(mac)
        if mac["destination_port"][0][:2] == "Te": # type: ignore # Ignore Ten Gig Ports
            continue
        mac_vendor = vendor(mac["destination_address"]) # type: ignore
        if mac_vendor == Vendor_to_change:
            if allready_dot1x(ssh,mac["destination_port"][0]): # type: ignore # Port Allready changed to dot1x
                continue
            change_interface(ssh,mac["destination_port"][0])   # type: ignore # Change Port
    print (f"{len(macs)} Mac Adresses checked")

if __name__ == "__main__":
    dot1x_work(seeddevice)
